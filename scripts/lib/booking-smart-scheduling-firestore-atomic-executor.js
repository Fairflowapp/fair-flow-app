/**
 * Smart Scheduling Phase 18E — Firestore Emulator atomic booking executor.
 *
 * Node-test only. Reuses Phase 18D evaluateAtomicMutation as the algorithm.
 ...
 * - appointment IDs are preallocated candidate refs, not Firestore auto-ids
 */
"use strict";
  ...
  async function executeAtomicMutation(mutation, execOpts) {
    ...
    while (outer < maxOuter) {
      ...
      if (writes.appointment) {
        tx.set(refs.appointments.doc(writes.appointment.appointmentId), writes.appointment);
      }
