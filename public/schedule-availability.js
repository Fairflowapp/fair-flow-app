...
function buildDefaultAvailabilityForDate(staff, dateKey) {
  ...
  const startTime = normalizeTimeValue(daySchedule.startTime);
  const endTime = normalizeTimeValue(daySchedule.endTime);
...
function getEffectiveAvailabilityForDate(staff, requests, dateKey) {
  ...
  const defaultAvailability = buildDefaultAvailabilityForDate(staff, normalizedDate);
...
function getEffectiveAvailability(staff, requests, dateRange) {
  ...
  const availability = dates.map((dateKey) => {
    const defaultAvailability = buildDefaultAvailabilityForDate(staff, dateKey);
