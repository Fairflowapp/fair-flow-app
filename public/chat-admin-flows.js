// chat-admin-flows.js
// Chat Flows admin builder (split of chat-admin.js). Verbatim move of the flow
// window.* handlers, draft collect/sync, save/edit/delete flow, and the flows
// admin-list + builder renderers. Core deps (permission gate, effective loc,
// flows loader) and the two resident category-field helpers are injected via
// initChatAdminFlows() so bodies stay unchanged and the import graph is acyclic
// (chat-admin.js -> chat-admin-flows.js only). window.* handlers remain global.
...
// ─── Injected core dependencies (set by initChatAdminFlows, fanned out from initChatAdmin) ──
let _chatManageAllowed = () => false;
let _chatEffectiveLocKey = () => '';
let loadChatFlows = async () => {};
let _ensureChatCategoryFields = () => {};
let _chatRefreshCategorySuggestions = () => {};

export function initChatAdminFlows(deps) {
  deps = deps || {};
  if (typeof deps._chatManageAllowed === 'function') _chatManageAllowed = deps._chatManageAllowed;
  if (typeof deps._chatEffectiveLocKey === 'function') _chatEffectiveLocKey = deps._chatEffectiveLocKey;
  if (typeof deps.loadChatFlows === 'function') loadChatFlows = deps.loadChatFlows;
  if (typeof deps._ensureChatCategoryFields === 'function') _ensureChatCategoryFields = deps._ensureChatCategoryFields;
  if (typeof deps._chatRefreshCategorySuggestions === 'function') _chatRefreshCategorySuggestions = deps._chatRefreshCategorySuggestions;
}
...
export { _renderFlowsAdminList, _renderFlowBuilder };
