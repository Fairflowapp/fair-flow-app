...
async function loadUserProfile() {
  ...
  try {
    ...
    if (snap.exists()) {
      const d = snap.data();
      const salonId = d.salonId || (typeof window !== "undefined" ? window.currentSalonId : null);
      let role = (d.role || "technician").toLowerCase();
      const staffId = d.staffId || user.uid;
      // Fallback: staff by staffId, or by email match, or members doc
      if (salonId && ["technician", ""].includes(role)) {
        try {
          ...
          if (staffSnap.exists()) {
            ...
            if (s.isAdmin === true) role = "admin";
            else if (s.isManager === true) role = "manager";
          }
          if (["technician", ""].includes(role)) {
            ...
            for (const docSnap of staffColl.docs) {
              ...
              if ((s.email || "").toLowerCase() === userEmail) {
                if (s.isAdmin === true) { role = "admin"; break; }
                if (s.isManager === true) { role = "manager"; break; }
              }
            }
          }
          if (["technician", ""].includes(role)) {
      ...
      // Ensure members doc exists so Storage rules (belongsToSalon) pass for uploads
      if (salonId) {
...
/** Apply tab visibility – Technician: no tabs, only + Upload Work. Manager/Admin: My Uploads + To Handle. */
function applyToHandleVisibility() {
  const role = (currentUserProfile?.createdByRole || "").toLowerCase();
  const isTechnician = !["manager", "admin", "owner"].includes(role);
  const showToHandle = ["manager", "admin", "owner"].includes(role);
  if (!showToHandle) currentMediaTab = "my_uploads";
  const tabsWrap = document.getElementById("mediaTabsWrap");
  if (tabsWrap) {
    tabsWrap.style.setProperty("display", isTechnician ? "none" : "flex", "important");
  }
  const toHandleBtn = document.getElementById("mediaTabToHandle");
  if (toHandleBtn) {
...
async function openWorkDetails(workId) {
  ...
  downloadBtn.onclick = async (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (!firstMedia?.mediaUrl && !firstMedia?.storagePath) { alert("No media to download"); return; }
    downloadBtn.disabled = true;
    downloadBtn.textContent = "…";
    let storagePath = firstMedia.storagePath;
    if (!storagePath && firstMedia.mediaUrl) {
      const m = firstMedia.mediaUrl.match(/\/o\/(.+?)(?:\?|$)/);
      if (m) storagePath = decodeURIComponent(m[1]);
    }
    const ext = (storagePath || "").match(/\.(jpe?g|png|gif|webp|mp4|webm)$/i)?.[1] || "jpg";
    ...
    try {
      ...
      if (storagePath && storage) {
        try {
          ...
          lastErr = err?.message || "getBlob failed";
        }
      }
      if (!blob) {
        try {
          const xhr = new XMLHttpRequest();
          xhr.open("GET", firstMedia.mediaUrl, true);
          ...
          lastErr = err?.message || "XHR failed";
        }
      }
      if (blob && blob.size > 0) {
      ...
      } else if (storagePath && storagePath.startsWith("salons/") && auth.currentUser) {
        try {
          const app = getApp();
          const fn = getFunctions(app, "us-central1");
          const getBase64 = httpsCallable(fn, "getMediaDownloadBase64");
          const res = await getBase64({ storagePath });
          const b64 = res?.data?.base64;
          const fName = res?.data?.fileName || fileName;
          if (b64) {
          ...
          lastErr = err?.details || err?.message || "Download failed";
          if (err?.message?.includes("too large") || err?.message?.includes("resource-exhausted")) {
            window.open(firstMedia.mediaUrl, "_blank");
            alert("File too large for direct download. Opened in new tab – right-click → Save.");
          } else {
            try {
              const app = getApp();
              const fn = getFunctions(app, "us-central1");
              const getUrl = httpsCallable(fn, "getMediaDownloadUrl");
              const res = await getUrl({ storagePath, fileName });
  ...
  console.log("[SelfDeleteCheck]", {
    role,
    uid,
    workId: work?.id || work?.workId,
    createdByUid: work?.createdByUid,
    postedCount: work?.postedCount,
  ...
  content.onclick = async (e) => {
    ...
    if (mediaId && isAdminUser && confirm("Delete this media item?")) {
      try {
...
export function initMediaUpload() {
  ...
  await loadUserProfile();
    ...
    // TO HANDLE only for Manager/Admin – Technician sees only My Uploads
    applyToHandleVisibility();
    const showToHandle = ["manager", "admin", "owner"].includes((currentUserProfile?.createdByRole || "").toLowerCase());
    if (showToHandle) {
