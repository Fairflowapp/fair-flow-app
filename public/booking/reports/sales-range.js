/**
 * Reports-safe sales range helpers.
 * Pure pagination, timezone bounds, and completeness metadata.
 * Firestore reads stay in the Sales repository.
 */
(function () {
  var PAGE_SIZE = 150;
  ...
  function emptyResult(extra) {
    ...
    return {
      ...
      fetchedCount: Number(row.fetchedCount) || (Array.isArray(row.sales) ? row.sales.length : 0),
  ...
  function compareSalesDesc(a, b) {
    var dt = closedAtMs(b && b.closedAt) - closedAtMs(a && a.closedAt);
    if (dt) return dt;
    return saleIdOf(b).localeCompare(saleIdOf(a));
  }

  function mergeSales(chunks) {
    var seen = {};
    var out = [];
  ...
  async function paginateRange(fetchPage, options) {
    if (typeof fetchPage !== "function") return failResult("Sales range lookup is not available.");
  ...
  function combineLocationResults(results) {
    ...
    list.forEach(function (row) {
      ...
      chunks.push(item.sales || []);
    });
    if (error) return failResult(error);
    var sales = mergeSales(chunks);
    return emptyResult({
      sales: sales,
      complete: complete,
      fetchedCount: sales.length,
  ...
  function userSafeError(error) {
    ...
    if (msg === "Choose a location." || msg === "Choose a start and end date." || msg === "Sales are not loaded.") {
      return msg;
    }
  ...
  function viewState(fetchResult) {
    var row = fetchResult || {};
    if (row.error) {
      return { kind: "error", message: userSafeError(row.error), sales: [] };
    }
    if (row.complete !== true) {
      return { kind: "incomplete", message: INCOMPLETE_MESSAGE, sales: [] };
    }
    var sales = Array.isArray(row.sales) ? row.sales : [];
    if (!sales.length) {
      return { kind: "empty", message: "No closed sales in this period.", sales: [] };
    }
  ...
  async function fetchForReport(repo, options) {
    ...
    if (!repo) return failResult("Sales are not loaded.");
    try {
      if (typeof repo.listForLocationsRange === "function") {
        return await repo.listForLocationsRange(ids, fromKey, toKey, opts);
      }
      if (typeof repo.listForLocationRange === "function") {
        var parts = await Promise.all(ids.map(function (id) {
          return repo.listForLocationRange(id, fromKey, toKey, opts);
        }));
        return combineLocationResults(parts);
      }
      return failResult("Sales range lookup is not available.");
    } catch (err) {
      return failResult(err && err.message ? err.message : LOAD_ERROR_MESSAGE);
    }
  }

  window.ffBookingReportsSalesRange = {
    PAGE_SIZE: PAGE_SIZE,
    ...
    compareSalesDesc: compareSalesDesc,
