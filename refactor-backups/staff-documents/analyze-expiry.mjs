...
console.log('=== expiry slab top-level fns ===');
console.log(' ', S.topFns.join(', '));
console.log('\n=== expiry module imports needed ===');
...
console.log('\n=== free idents in slab not from known imports (globals/injected) ===');
console.log('  ', [...S.used].filter(n=>!src[n] && !S.topFns.includes(n)).sort().join(', '));
console.log('\n=== slab fns still used by main-after (=> main imports from expiry) ===');
...
for(const[s,v]of Object.entries(IMPORTS)){
  ...
  console.log(`  [${s}] orphan: ${orphan.join(', ')||'(none)'}`);
}
